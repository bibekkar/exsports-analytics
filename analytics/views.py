...
def getStats(batsman, bowler):
    ...
    url = "https://iplstatbk2.azurewebsites.net/api/head2head/"
    url = url + batsman + '/' + bowler
    # print(url)
    response = requests.get(url)  #https://iplstatbk2.azurewebsites.net/api/head2head/MS Dhoni/SL Malinga
    # print(response)
    html = response.text
    # print(html)
    html_tup = eval(html)
    ...
    res['Runs'] = html_tup[0]
    res['Balls'] = html_tup[1]
    res['Outs'] = html_tup[3]
    res['Dots'] = html_tup[4]
    res['Fours'] = html_tup[5]
    res['Sixes'] = html_tup[6]
    res['Strike Rate'] = html_tup[2]

    return res

def index(request):
    deliveries_dataset = os.path.join(settings.BASE_DIR, 'data', 'dataset_deliveries.csv')


    df_deliveries = pd.read_csv(deliveries_dataset)
    ...
    if request.method == "GET":
        context_data = {
            ...
            'batsmanlist': batsman_list,
            'bowlerlist': bowler_list
        }
    else:
